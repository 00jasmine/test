...
import com.team.entity.StaffLeave;
import com.team.vo.StaffLeaveVO;
...
public interface StaffLeaveMapper extends BaseMapper<StaffLeave> {
    ...
    @Select("select * from hrm.att_staff_leave where is_deleted = 0 and staff_id = #{id}")
     ...
     * @return
     */
    @Select("select asl.*,ss.code,ss.name ,ss.phone,sd.name dept_name from hrm.att_staff_leave asl inner join hrm.sys_staff ss on asl.staff_id = ss.id " +
            "inner join hrm.sys_dept sd on ss.dept_id = sd.id " +
